...
function bskEncrypt(plaintext, masterPassword) {
  const length = plaintext.length;
  const key = keyGeneration(length, masterPassword);
  const ciphertext = encrypt(plaintext, key);
  return ciphertext;
}

function bskDecrypt(ciphertext, masterPassword) {
  if (!ciphertext) return "";

  let Mlength = ciphertext.length / 2;
  let pad = pad_bytes(ciphertext);
  Mlength = Mlength - pad;

  const key = keyGeneration(Mlength, masterPassword);
